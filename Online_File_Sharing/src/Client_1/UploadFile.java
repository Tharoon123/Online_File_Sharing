/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package Client_1;

import java.io.IOException;
import java.nio.ByteBuffer;
import java.nio.channels.FileChannel;

import java.nio.file.Paths;
import java.nio.file.StandardOpenOption;

/**
 *
 * @author This PC
 */
public class UploadFile{
    
    void upload( String srcPath, String srcName){
        Up_Progress_Bar load = new Up_Progress_Bar();
        load.uploading();
        /*Upload_Complete ok=new Upload_Complete();
        ok.setVisible(true);*/
        
        String srcAdd = srcPath;
        String destAdd = "C:\\Users\\This PC\\Documents\\NetBeansProjects\\Online_File_Sharing\\src\\Server\\Database\\"+srcName;
        
        System.out.println(destAdd);
        try {
            FileChannel srcChnl = FileChannel.open(Paths.get(srcAdd), StandardOpenOption.READ);
            FileChannel destChnl = FileChannel.open(Paths.get(destAdd), StandardOpenOption.CREATE, StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING);
            ByteBuffer buff = ByteBuffer.allocate(1024);
            
            
            while(srcChnl.read(buff)!=-1 || buff.position()>0){
                buff.flip();
                while(buff.hasRemaining()){
                    destChnl.write(buff);
                }
                buff.clear();
            }
            srcChnl.close();
            destChnl.close();
            
            Upload_Complete upCl1=new Upload_Complete();
            upCl1.setVisible(true);
            
            
                    
            
            System.out.println("File Uploaded SuccessFully.....!!!!");
                
            
        } catch (IOException e) {
            System.out.println("Please Select a file ");
            Upload_Error err= new Upload_Error();
            err.setVisible(true);
            e.printStackTrace();
        }
        
        
        
    }
    
}


